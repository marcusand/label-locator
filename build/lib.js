 ...
 * @returns
 */
export function coolingSchedule(currT, initialT, sweeps) {
 ...
 * @returns
 */
export function rectOverlap(rectA, rectB) {
    ...
    const xOverlap = Math.max(0, Math.min(rectA.x2, rectB.x2) - Math.max(rectA.x1, rectB.x1));
    const yOverlap = Math.max(0, Math.min(rectA.y2, rectB.y2) - Math.max(rectA.y1, rectB.y1));
    return xOverlap * yOverlap;
}
export function getLabelAnchorRootOverlap(label, anchor, labelMargin, anchorMargin) {
    return rectOverlap({
...
export function getLabelLabelOverlap(labelA, labelB, labelMargin) {
    return rectOverlap({
